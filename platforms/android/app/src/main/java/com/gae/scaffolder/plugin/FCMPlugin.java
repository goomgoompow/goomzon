...
public class FCMPlugin extends CordovaPlugin {
	...
	public static Boolean isDeviceRead = false;
	...
	public boolean execute(final String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException {
		...
		try{
				...
				cordova.getActivity().runOnUiThread(new Runnable() {
					public void run() {
						try{
							...
							Log.d(TAG,"\t[FCMPlugin.java]: execute : action[getToken] Token: "+ token);
						}catch(Exception e){
							Log.d(TAG,"\tError retrieving token");
							callbackContext.error("Error retrieving token");
						}
					}
				});
			}
			// NOTIFICATION CALLBACK REGISTER //
				...
				cordova.getActivity().runOnUiThread(new Runnable() {
					public void run() {
						Log.d(TAG, "[FCMPlugin.java] registerNotification_run: lastPush = "+lastPush);
